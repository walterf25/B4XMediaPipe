...
Public Sub PrepareSurfaceForVideo (MyTaskIndex As Int, Dir As String, FileName As String) As ResumableSub
	...
	Camera.StartSession(tv, PreviewSize, CaptureSize, 0, 256, True)
	Wait For Camera_SessionConfigured (Success As Boolean)
	If MyTaskIndex <> TaskIndex Then Return False
	Return Success
End Sub
...
Private Sub GetHintOrientation As Int
	...
	Dim Front2 As Int = -1
	If getIsFrontFacingCamera Then Front2 = 1
	Dim orientation As Int = (SensorOrientation + jcamera.GetField("lastKnownOrientation") * Front2 + 360) Mod 360
	Return orientation
End Sub

'''Sub Camera_PreviewTaken (data() As Byte)
'''	
'''End Sub

Sub Camera_DataAvailable (data() As Byte)
	'''Log("preview taken... " & data.Length)
	Dim bmp As Bitmap = DataToBitmap(data)
'''	B4XPages.MainPage.DataAvailable(bmp)
End Sub
...
Private Sub updatePreview(previewBuilder As JavaObject)
	previewBuilder.RunMethod("set", Array(getControlMode2(previewBuilder), 1))
End Sub
...
End Sub
'Gets or sets the control mode (one of the CONTROL_MODE values). Call StartPreview to apply changes.
Public Sub setControlMode(Mode As String)
	SetBothMaps("CONTROL_MODE", CONTROL_MODE.IndexOf(Mode))
End Sub
...
public Sub getControlMode2(builder As JavaObject) As String
	Log("intToConst: " & IntToConst(GetFromCaptureRequest(builder, "CONTROL_MODE"), CONTROL_MODE))
	Return IntToConst(GetFromCaptureRequest(builder, "CONTROL_MODE"), CONTROL_MODE)
End Sub
